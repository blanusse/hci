...
export const HornoCardActions = defineComponent({
   props: { device: { type: Object, required: true } },
   emits: ['update:state'],
   setup(props, { emit }) {
      const encendido = ref(props.device.state?.status === 'on')
...
export const HeladeraCardActions = defineComponent({
   props: { device: { type: Object, required: true } },
   emits: ['update:state'],
   setup(props, { emit }) {
      const encendido = ref(props.device.state?.status === 'on')
...
export const PersianaCardActions = defineComponent({
   props: { device: { type: Object, required: true } },
   emits: ['update:state'],
   setup(props, { emit }) {
      ...
      return () => h('button',
         { class: ['card-action-btn', { active: abierta.value }], onClick: toggle },
         [
            h('svg', { width: 14, height: 14, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': 2 }, [
               h('path', { d: 'M3 3h18v2H3z' }),
               h('path', { d: 'M12 5v16' }),
               h('path', { d: 'M8 5l-2 16' }),
               h('path', { d: 'M16 5l2 16' }),
               h('path', { d: 'M4 21h16' })
